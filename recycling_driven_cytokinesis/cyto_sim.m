...
nbins =50;
Nb = 1/errorer^2;
simsteps = floor(Nb*nbins/10*(L0/sig)^2*log(Nb));
totalsteps = 1*simsteps;
l = F*x;
...
tic
fcount = 0;
while ind < totalsteps +1
   fprintf(' %d %d %d \n',ind, fcount, totalsteps);  
    oldx = x;
    x = x + normrnd(0,sig,size(x));%rand(size(x));
    l = F*x;
    if(sum(l>=L0)==0&&sum(l<=0)==0)
        G = -sum(log(L0-l)+l/(L0-L)-1./l)*L0/lp + K + c/2*sum((C*x - Cl*exp(-mod(ind,simsteps)/simsteps*10)).^2) + b/2*sum((B.*x+D).^2);
        oldl = F*oldx;
...
mint = floor(1/100*(L0/sig)^2*log(Nb));
stoP(stot<mint)=[];
stol(stot<mint)=[];
stoQ(:,stot<mint)=[];
stob(stot<mint)=[];
...
l_ff = linspace(min(l_f),max(l_f),nbins);

[f, ls] = hist(myl(1:mint:end,:),nbins);
